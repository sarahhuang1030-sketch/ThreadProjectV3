import db from "./lib/database_nk.js";
//get all customers
...
export async function getCustomerById(id) {
  try { 
    ...
    const [customers] = await db.query("SELECT * FROM customers WHERE CustomerId = ?", [id]);
...
export async function createCustomer({
  CustFirstName,        
    ...
    if (!CustFirstName?.trim() || !CustLastName?.trim() || !CustAddress?.trim() || !CustCity?.trim() || !CustProv?.trim() || !CustPostal?.trim() || !CustCountry?.trim() || !CustHomePhone?.trim() || !CustBusPhone?.trim() || !CustEmail?.trim())
...
//update customer
export async function updateCustomer(id, {
    CustFirstName,
    CustLastName,
    CustAddress,        
    CustCity,
    ...
    const result = await db.query(
        ...
        [CustFirstName, CustLastName, CustAddress, CustCity, CustProv, CustPostal, CustCountry, CustHomePhone, CustBusPhone, CustEmail, id]
    );
